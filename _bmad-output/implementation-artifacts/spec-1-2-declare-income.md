...
- **medium / patch** — `incomes_test.go`'s handler tests (`TestUpsertIncome_HappyPath`/`_Idempotency`) use `sqlmock.AnyArg()` for the `cycle_start_date` argument on both the INSERT and the canned re-SELECT, and never assert the response's `cycleStartDate` — so a future slip at `incomes.go:36` (e.g. swapping `CycleWindow`'s `start`/`end` return values, which the code comment itself flags as the exact line Story 1.4 will touch) would silently ship every income one cycle late while the full test suite stayed green. Verified via demonstration in the review. [verification-gap]
- **low / patch** — `PreviousCycles` (cycle.go) panics (`makeslice: cap out of range`) for `n < 0`, with no guard and no doc/test establishing the contract, despite being billed as code every later story/epic imports. Fix is a direct one-line early return. [blind-hunter, edge-case-hunter]
- **low / patch** — `TestPreviousCycles` only walks back through ordinary months; nothing pins the backward walk together with a clamped short-month boundary (e.g. `cycleStartDay=31` stepping back through February), the exact interaction `CycleWindow`'s own tests call out as needing coverage. Manually traced and confirmed correct, but unverified by an automated test. [blind-hunter]
- **low / patch** — `repo_income_test.go` has no test for `IncomeRepo.Upsert`'s post-write re-SELECT returning a genuine (non-"not found") error — the fallback-to-inserted-values branch mirrored from `BudgetRepo.Upsert` is exercised by no test, same gap class Story 1.1 patched for other repos. [blind-hunter]
- **low / patch** — `Income` struct lives in `incomes.go`, breaking the established convention (`User`/`Category`/`Transaction`/`Budget` all live centrally in `api.go`) — a reader following that convention won't find it. Fix is a direct move, no logic change. [blind-hunter]
