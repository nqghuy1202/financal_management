...
1. **Folder+id dispatch** (`{spec_folder}` and `{story_id}` are set): the write-back always lands at the id-keyed story spec. The `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts` fallback in step 2 below is never used in this mode, even for halts before planning starts.
...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/bmad-build-auto-result-<slug-or-timestamp>.md` with:
     ```markdown
     ---
     status: <final status>
     ---
...
Read fully and follow: `D:\Personal_Project\financal_management\_bmad\render\bmad-build-auto\financal-management-6513de358694\552a99337aa2879dc4f3\step-01-clarify-and-route.md`.
