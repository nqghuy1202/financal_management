...
If `{story_key}` is not empty and `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\sync-sprint-status.md` with `{target_status}` = `in-progress`.
...
Write what triggered the stop in `## Implementation Notes`. Then update `{spec_file}`: add back `## Code Map` (filled in from what you learned while implementing) and `## Open Questions` (one question per gap), set `route: 'dispatch'` and `status: 'draft'`. Go back to `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-02-plan.md` step 6.
...
If a layer needs subagents and you cannot launch them, write the full prompt for each layer under `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts` (with placeholders filled in, not just file paths). Stop and ask the user to run each prompt in a separate session and paste back the findings.
...
- **defer** — Everything else: old bugs not caused by this change, ideas for later, groups where every member is `maybe-false` and would be `medium` or `high` if true (record that severity marked unverified, and what would prove it; if it would only be `low`, reject it), or fixes that would edit CLAUDE.md, AGENTS.md, rules, or specs. Add one entry to `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/deferred-work.md`:
...
If `{story_key}` is not empty and `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/sprint-status.yaml` exists, read `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\sync-sprint-status.md` with `{target_status}` = `review`.
