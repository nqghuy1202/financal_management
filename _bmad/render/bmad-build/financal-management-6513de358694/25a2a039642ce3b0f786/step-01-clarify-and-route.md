...
1. Explicit argument
   ...
   - If the user explicitly supplied a spec folder and a story id, with no specific spec file path, set `spec_folder` and `story_id`. Read `{spec_folder}/stories.yaml`; if it is missing or fails to parse, HALT rather than falling back to `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts`. Find the one entry whose string `id` exactly equals `story_id`; if none exists, HALT rather than falling back. Use that entry's `title` and `description` as the starting intent.
   ...
   - If it points to a file that matches the spec template (has `status` frontmatter with a recognized value: draft, ready-for-dev, in-progress, in-review, or done) → set `spec_file`. Before exiting, run **Story-key resolution** (below). Then **EARLY EXIT** to the appropriate step: `draft` → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-02-plan.md`, `ready-for-dev`/`in-progress` → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-03-implement.md` (or `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-oneshot.md` when `route` is `oneshot`), `in-review` → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-04-review.md`. For `done`, ingest as context and proceed to INSTRUCTIONS — do not resume.
...
3. Otherwise — scan artifacts and ask
   - Active specs (`draft`, `ready-for-dev`, `in-progress`, `in-review`) in `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts`? → List them and HALT. Give the user a choice:
     - Resume one of the listed specs
     - **New** — start new work
     If `draft` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-02-plan.md` (resume planning from the draft)
     If `ready-for-dev` or `in-progress` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-03-implement.md` (or `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-oneshot.md` when `route` is `oneshot`)
     If `in-review` selected: Set `spec_file`. Run **Story-key resolution** (below). **EARLY EXIT** → `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\step-04-review.md`
...
1. Load context.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     2. **Check for a valid cached epic context.** Look for `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/epic-<N>-context.md` (where `<N>` is the epic number). A file is **valid** when it exists, is non-empty, starts with `# Epic <N> Context:` (with the correct epic number), and no file in `D:\Personal_Project\financal_management/_bmad-output/planning-artifacts` is newer.
     ...
     3. **Compile epic context.** Produce `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/epic-<N>-context.md` by following `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\compile-epic-context.md`, in order of preference:
        - **Preferred — subagent:** spawn a subagent synchronously (wait for it to return in this turn) with `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\compile-epic-context.md` as its prompt. Pass it the epic number, the epics file path, the `D:\Personal_Project\financal_management/_bmad-output/planning-artifacts` directory, and the output path `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/epic-<N>-context.md`.
        - **Fallback — inline** (for runtimes without subagent support, e.g. Copilot, Codex, local Ollama, older Claude): if your runtime cannot spawn subagents, or the spawn fails/times out, read `D:\Personal_Project\financal_management\_bmad\render\bmad-build\financal-management-6513de358694\25a2a039642ce3b0f786\compile-epic-context.md` yourself and follow its instructions to produce the same output file.
...
4. Multi-goal check (see SCOPE STANDARD). If the intent fails the single-goal criteria:
   ...
   - If the user chooses **Split**: For each deferred goal, append one new entry to `D:\Personal_Project\financal_management/_bmad-output/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Narrow scope to the first-mentioned goal. Continue routing.
     ```markdown
     - source_spec: none
